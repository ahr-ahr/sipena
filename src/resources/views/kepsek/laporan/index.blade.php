    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <div class="flex gap-1 border
                ...
                <button id="btn-table"
                    class="px-3 py-2 text-sm bg-gray-100 dark:bg-white/10">
                    Table
                </button>
                <button id="btn-card"
...
<div id="table-view"
    ...
    <div id="pagination"
         class="p-4 flex justify-end gap-2"></div>
</div>
...
btnCard.onclick = () => setView('card')

function loadData(page = 1) {
    const params = new URLSearchParams({
        q, kategori_id: kategori, status,
        sort_by: sortBy, sort_dir: sortDir,
        page
    })

    fetch(`/kepsek/laporan?${params.toString()}`, {
        ...
        tbody.innerHTML += `
            <tr class="border-b">
                ...
                <td class="px-4 py-2 text-right space-x-2">
                    ...
                    ${l.status === 'disetujui_tu' ? `
                        <button onclick="sekepsekjui('${l.uuid}')"
        ...
        card.innerHTML += `
            <div class="p-4 rounded-xl border space-y-2">
                ...
                <div class="flex gap-2 pt-2">
                    ...
                    ${statusBadge(l.status === 'disetujui_tu' ? `
                        <button onclick="sekepsekjui('${l.uuid}')"
                            class="text-green-600 text-xs">
                            Setujui
                        </button>
...
function sekepsekjui(uuid) {
    fetch(`/kepsek/laporan/${uuid}/sekepsekjui`, {
        method: 'PATCH',
        headers: {
...
function tolak(uuid) {
    fetch(`/kepsek/laporan/${uuid}/tolak`, {
        method: 'PATCH',
        headers: {
            'X-CSRF-TOKEN': '{{ csrf_token() }}',
            'Accept': 'application/json'
...
function buildExportUrl(base) {
    const params = new URLSearchParams({
        q, kategori_id: kategori,
        status, sort_by: sortBy,
        sort_dir: sortDir,
    })
...
function exportExcel() {
    window.location = buildExportUrl('/kepsek/laporan/export/excel')
}
function exportPdf() {
    window.location = buildExportUrl('/kepsek/laporan/export/pdf')
}
